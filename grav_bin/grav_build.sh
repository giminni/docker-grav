...
main() {
   ...
   if [ "${_GRAV_TAG}" == "latest" ]; then 
      if [ ! -f ${CFG_DIR}/.config.prod ] || [ ! -n $(cat ${CFG_DIR}/.config.prod | tr -d '"' | cut -d'=' -f2) ]; then error 2 "FAIL: Grav prod version not provided! Please run ${BIN_DIR}/grav_getver.sh first..."; fi
   fi

   # Check if version string is set for development
   if [ "${_GRAV_TAG}" == "testing" ]; then 
      if [ ! -f ${CFG_DIR}/.config.dev ] || [ ! -n $(cat ${CFG_DIR}/.config.dev | tr -d '"' | cut -d'=' -f2) ]; then error 2 "FAIL: Grav dev version not provided! Please run ${BIN_DIR}/grav_getver.sh first..."; fi
   fi
